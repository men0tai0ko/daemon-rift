...
### v0.9.0（2026-05-08）
- パーティHPストリップを戦闘ログ直下に移動（視認性改善）
- パーティ隊列変更: 仲魔画面に ↑/↓ ボタン追加。戦闘中交代パネルに倉庫仲魔を追加（戦死枠と交代可能）
- 属性弱点インジケータ: 戦闘中に先頭仲魔の属性と敵属性の相性（有利/不利）を常時表示
- ストリップカードタップでスキル確認: toastでスキル名一覧を表示
- ボスフロア接近警告: progress=4 かつ次フロアがボス時にログ警告
- 合体プレビューにステータス追加: HP/ATK/DEF/SPD を表示
- パーティバーとパーティ画面に LEAD 表示追加（最前衛マーカー）
- 逃走確率をボタンに明示（60%）
