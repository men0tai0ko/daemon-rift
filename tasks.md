...
**最終更新:** 2026-05-10  
**現在のバージョン:** v0.11.0（パーティ・合体・アイテム改善）
...
### v0.11.0（2026-05-10）
- **改善**: 戦死した仲魔が自動的にパーティから倉庫へ移動（`_removeDeadFromParty()`追加）
- **改善**: スキルパネルで全生存仲魔を選択してスキルを使用可能に（performer指定追加）
- **改善**: 合体ラボで仲魔1体目タップ→A、2体目タップ→Bに自動割り当て。選択済みタップで解除、スロットタップで解除
- **改善**: 仲魔一覧→合体ラボ直接移動ボタン追加
- **改善**: 合体ラボ→仲魔一覧直接移動ボタン追加
- **改善**: 探索中は合体ボタンをグレーアウト。探索停止で復活
- **改善**: 回復アイテム（回復薬・万能薬）の対象選択パネルを戦闘・仲魔一覧の両方で追加
- **改善**: パーティ最後の1体を外せないよう保護
- **改善**: 倉庫からの加勢が空きパーティ枠に追加する方式に変更

### v0.10.0（2026-05-10）
- **バグ修正**: 全仲魔が攻撃に参加するよう修正（先頭のみ問題を解消）
- **バグ修正**: 敵攻撃がランダムな生存仲魔を対象にするよう修正
- **バグ修正**: 合体画面で同名悪魔を HP で区別可能に（スロット・一覧ともに HP:現在/最大 表示）
- 戦闘ログ色分け: プレイヤー行（シアン）・敵行（赤）・システム行（紫）
- タイトル画面に最高到達フロア記録を表示
- 敵HP%をリアルタイム表示
- 合体確認ダイアログ: 2タップ確定（初回タップで⚠表示、3秒でリセット）
- スキルパネルに攻撃スキルの予想ダメージ範囲を表示
- スキル使用ログにスキル使用者名を追記
- 仲魔画面に EXP 現在/次レベルを表示
- 探索中 HP が 25% 以下になった仲魔に危機ログを表示（重複防止付き）
